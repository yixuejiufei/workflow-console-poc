...
- **v0.1.55**：【设置】引擎模式下「默认模型」改为下拉选择 litellm 可用模型——数据源 = 引擎代理端点 `GET /api/v1/settings/llm/models`（**issue-093**，引擎实现后生效）：引擎内部用完整 key 拉 litellm `/v1/models` + 逐个实测过滤（litellm `/key/info` 白名单含假阳性，如 kimi-for-coding 配置在 key 上但实际 403；只有实测 200 的模型才真正可用），返回 `[{id, available}]`。前端不接触完整 key（引擎 `/settings/llm` 返回掩码 key，直连 litellm 会 401——实测）。下拉中 ✅=可用、⚠️=无权限（置灰禁选）、当前值不在列表时显示「自定义: xxx」；✎ 按钮切换回手动输入（非 engine 模式保持原输入框）。引擎未实现时优雅降级为手动输入
